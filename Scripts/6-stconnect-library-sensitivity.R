...
## February 28, 2024
##
## This script generates a stconnect library for sensitivity analyses on mean
## effective resistance distance for two parameters: 
##    - resistance map resolution
##    - number of sentinel nodes
## It is based on a single iteration and timestep, one protected area scenario, 
## and three species representative of diverse habitat needs and movement 
## abilities (American Marten, Red Backed Salamander, Red-breasted Nuthatch).
##
## The library is available to collaborators from SyncroSim Cloud at 
## 
## 
## NOTE: In order to run this script, please review line 53 to properly setup
...
stconnectLibrary <- ssimLibrary(
  name = file.path(libraryDir, 
                   "a311-stconnect-sensitivity-resolution.ssim"),
...
# Landscape change & Habitat Suitability transformers
...
# Circuitscape transformer
targetScenario <- edit_subscenario(
  scenarioName = "Pipeline: Circuitscape",
  scenarioInput = "Pipeline - Circuitscape.csv",
  scenarioDatasheet = "core_Pipeline")
# Move scenario into folder
folderId(targetScenario) <- folderId(folder(stconnectProject, "00 Pipeline"))
...
# Random
targetScenario <- edit_subscenario(
  scenarioName = "Transition Spatial Multiplier: Random",
  scenarioInput = "Transition Spatial Multipliers - Random.csv",
  scenarioDatasheet = "stsim_TransitionSpatialMultiplier")
# Move scenario into folder
folderId(targetScenario) <- folderId(spatialMultiplierFolder)
...
# Create folder
sensitivityScenarioFolder <- folder(stconnectProject, 
                                    folder = "Sensitivity Scenarios")

# Create nested sub-folders
agnosticFolder <- folder(stconnectProject,
                         folder = "Agnostic",
                         parentFolder = "Sensitivity Scenarios")
zonationFolder <- folder(stconnectProject,
                         folder = "Zonation",
                         parentFolder = "Sensitivity Scenarios")
prioritizrFolder <- folder(stconnectProject,
                       folder = "Prioritizr",
                       parentFolder = "Sensitivity Scenarios")
randomFolder <- folder(stconnectProject,
                       folder = "Random",
                       parentFolder = "Sensitivity Scenarios")

# Landscape Change & Habitat Suitability
targetScenario <- create_fullscenario(
  scenarioName = "Landscape Change & Habitat Suitability - Agnostic",
  destinationFolder = agnosticFolder,
  dependenciesCore = c(1, 3:8, 9, 13:19),
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Landscape Change & Habitat Suitability - Zonation",
  destinationFolder = zonationFolder,
  dependenciesCore = c(1, 3:8, 10, 13:19),
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Landscape Change & Habitat Suitability - Prioritizr",
  destinationFolder = prioritizrFolder,
  dependenciesCore = c(1, 3:8, 11, 13:19),
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Landscape Change & Habitat Suitability - Random",
  destinationFolder = randomFolder,
  dependenciesCore = c(1, 3:8, 12, 13:19),
  dependenciesUnique = NA)

# Run scenario
# NOTE: The scenarios were ran manually in rsyncrosim so that the Habitat 
#       Suitability transformer could be run only for 2060 and for the three key
#       species.

# Circuitscape -----------------------------------
# NOTE: Confirm the ID of the results scenario (i.e., 21 & 39 below)

# Agnostic
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 90 m resolution - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 180 m resolution - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 540 m resolution - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 50 nodes - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 35 nodes - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 2 nodes - Agnostic",
  destinationFolder = NA,
  dependenciesCore = c(20, 25, 2), 
  dependenciesUnique = NA)
...
# Zonation
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 90 m resolution - Zonation",
  destinationFolder = zonationFolder,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 180 m resolution - Zonation",
  destinationFolder = zonationFolder,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution - Zonation",
  destinationFolder = zonationFolder,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 540 m resolution - Zonation",
  destinationFolder = zonationFolder,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 50 nodes - Zonation",
  destinationFolder = NA,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 35 nodes - Zonation",
  destinationFolder = NA,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 2 nodes - Zonation",
  destinationFolder = NA,
  dependenciesCore = c(20, 26, 2), 
  dependenciesUnique = NA)
...
# Random
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 90 m resolution - Random",
  destinationFolder = randomFolder,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 180 m resolution - Random",
  destinationFolder = randomFolder,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution - Random",
  destinationFolder = randomFolder,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 540 m resolution - Random",
  destinationFolder = randomFolder,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 50 nodes - Random",
  destinationFolder = NA,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 35 nodes - Random",
  destinationFolder = NA,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)
targetScenario <- create_fullscenario(
  scenarioName = "Circuitscape: 270 m resolution, 2 nodes - Random",
  destinationFolder = NA,
  dependenciesCore = c(20, 28, 2), 
  dependenciesUnique = NA)

# NOTE: The stconnect package does not have resolution as an input. Therefore,
#       the scenarios above were ran manually in rsyncrosim so that the 
#       resistance maps were aggregated to the target resolution and 
#       disaggregated to the original resolution.
